...
//This function displays the number off all the total items in cart.
...
//This function calculate the Total Price of all the Items in the cart.
function setTotal(){
    let total = 0;
...
//This function Decreases the quantity of a specific product in the cart
function minus(){
    let id = this.parentNode.getAttribute('row-id');
    let prod = cart.find(item => item.id == id)
    if(prod.quantity === 1){
...
//This function Remove a specific Product from cart when the remove button is clicked
...
// This function will create a table row for a product when the product is added to the cart.
// It also set up eventListener for every buttons in the created row.
...
// this Function is Responsible for Loading all datas such as the price,
// name, image source and Id of all products to the home page..
function loadData(){
    let data = '';
    let text;
...
// This function validates the number of the user
